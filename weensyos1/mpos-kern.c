...
static pid_t thread_create(process_t *parent);

static int notify_blocked_processes(process_t *current)
{
	int flag = -1; // No waiting processes
	if (!current) {
		return -1;
	}

	while (current->index != 0) {
		pid_t p_pid = current->waiting_pid[current->index--];
		process_t *parent = &proc_array[p_pid];
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if (!notify_blocked_processes(current)) {
	...
	case INT_SYS_WAIT: {
		...
		/* cursorpos = console_printf(cursorpos, 0x0700, "wait called on %d " */
		/* 			   "by %d\n", */
		/* 			   p, current->p_pid); */
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			proc_array[p].index += 1;
			proc_array[p].waiting_pid[proc_array[p].index] =
				current->p_pid;
			/* cursorpos = */
			/* 	console_printf(cursorpos, 0x0700, "blocking %d on %d " */
			/* 		       "with index = %d\n", */
			/* 		       current->p_pid, p, */
			/* 		       proc_array[p].index); */
		}
	...
	case INT_SYS_THREAD: {
		/* cursorpos = console_printf(cursorpos, 0x0700, "in INT_SYS_THREAD %p\n", */
...
static int
copy_reg(process_t *dest, process_t *src)
{
	if (!dest || !src) {
		return -1;
	}

	dest->p_registers.reg_edi = src->p_registers.reg_edi;
	dest->p_registers.reg_esi = src->p_registers.reg_esi;
	dest->p_registers.reg_ebp = src->p_registers.reg_ebp;
	dest->p_registers.reg_original_esp = src->p_registers.reg_original_esp;
	dest->p_registers.reg_ebx = src->p_registers.reg_ebx;
	dest->p_registers.reg_edx = src->p_registers.reg_edx;
	dest->p_registers.reg_ecx = src->p_registers.reg_ecx;
	dest->p_registers.reg_eax = src->p_registers.reg_eax;

	dest->p_registers.reg_es = src->p_registers.reg_es;
	dest->p_registers.reg_padding1 = src->p_registers.reg_padding1;
	dest->p_registers.reg_ds = src->p_registers.reg_ds;
	dest->p_registers.reg_padding2 = src->p_registers.reg_padding2;

	dest->p_registers.reg_intno = src->p_registers.reg_intno;
	dest->p_registers.reg_err = src->p_registers.reg_err;
	dest->p_registers.reg_eip = src->p_registers.reg_eip;
	dest->p_registers.reg_cs = src->p_registers.reg_cs;
	dest->p_registers.reg_padding3 = src->p_registers.reg_padding3;
	dest->p_registers.reg_eflags = src->p_registers.reg_eflags;

	dest->p_registers.reg_esp = src->p_registers.reg_esp;
	dest->p_registers.reg_ss = src->p_registers.reg_ss;
	dest->p_registers.reg_padding4 = src->p_registers.reg_padding4;
...
do_fork(process_t *parent)
{
	...
	/* cursorpos = console_printf(cursorpos, 0x0700, */
	/* 			   "Found an empty entry in process descriptor " */
	/* 			   "table at position %d pid = %d.", i, */
	/* 			   spawned_proc->p_pid); */
